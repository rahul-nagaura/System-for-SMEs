...
export default function ProductTracking() {
  ...
  return (
    <section ref={sectionRef} className="bg-[#111111] py-16 md:py-24 px-5">
      <div className="max-w-[640px] mx-auto">
        <h2 className="text-4xl font-medium text-center text-white mb-7">Case Studies...</h2>
        <div
          className="rounded-[28px] overflow-hidden border border-white select-none"
          style={{ touchAction: "pan-y" }}
          onTouchStart={onTouchStart}
          onTouchEnd={onTouchEnd}
        >
          {/* Problem — black */}
          <div className="bg-[#111111] text-white px-6 md:px-9 pt-7 pb-9">
            <span className="w-11 h-11 rounded-full bg-white text-[#111111] flex items-center justify-center font-bold text-lg">
              {c.n}
            </span>
            <div className="mt-5 text-[11px] font-semibold uppercase tracking-[0.35em] text-[#4A4A4A]">
              {c.category}
            </div>
          ...
          {/* Solution — cream */}
          <div className="px-6 md:px-9 pt-8 pb-7" style={{ backgroundColor: "#FFFFFF" }}>
            <h3 className="text-[30px] md:text-[34px] font-medium leading-[1.14] text-[#111111]">{c.solution}</h3>
            ...
            {expanded && (
              <div className="mt-7">
                <div className="text-[15px] text-[#111111]/60">Client:</div>
                <div className="mt-1 text-2xl font-bold text-[#111111] leading-tight">{c.client}</div>
                <div className="text-[15px] text-[#111111]/55">{c.location}</div>
                <p className="mt-5 text-[15px] leading-relaxed text-justify text-[#111111]/80">{c.detail}</p>
              </div>
            )}
            ...
            <div className="mt-8 flex gap-4">
              <button
                ...
                className="flex-1 rounded-full py-4 font-medium text-lg bg-[#111111] text-white hover:opacity-90 active:scale-95 transition-all"
              >
                {expanded ? "Show Less" : "See Details"}
              </button>
              <button
                onClick={() => setZoomSrc(c.image)}
                className="flex-1 rounded-full py-4 font-medium text-lg text-[#111111] hover:opacity-90 active:scale-95 transition-all"
          ...
          <div className="mt-8 flex justify-center gap-2">
            {CASES.map((cs, i) => (
              <button
                ...
                className={`h-2.5 rounded-full transition-all duration-300 ${i === active ? "w-6" : "w-2.5"}`}
                style={{ backgroundColor: i === active ? GOLD : "rgba(255,255,255,0.25)" }}
              />
            ))}
          </div>
        )}
        ...
        >
          <div className="relative max-w-[1100px] w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              onClick={() => setZoomSrc(null)}
              aria-label="Close"
              className="absolute -top-4 -right-4 w-10 h-10 rounded-full bg-white text-[#111111] text-xl font-bold flex items-center justify-center shadow-lg hover:scale-105 transition-transform"
            >
              ✕
            </button>
          </div>
