  ...
  return (
    ...
    >
      <svg width="26" height="26" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <circle cx="12" cy="8" r="4" fill="#111111" fillOpacity="0.5" />
        <path d="M4 20c0-4.4 3.6-7 8-7s8 2.6 8 7" fill="#111111" fillOpacity="0.5" />
      </svg>
    </span>
  );
}
  ...
  return (
    <section ref={sectionRef} className="bg-white py-16 md:py-24 px-5">
      <div className="max-w-[640px] mx-auto">
        <h2 className="text-4xl font-bold text-center tracking-tight text-[#111111]">Testimonials</h2>
        ...
        >
          <div className="flex items-center gap-4">
            <Avatar index={active} />
            <div>
              <div className="text-lg font-bold text-[#111111] leading-tight">{r.name}</div>
              <div className="mt-0.5 text-[13px] font-medium text-[#111111]/45">{r.role}</div>
            </div>
          </div>
          <p className="mt-5 text-[14px] leading-relaxed text-justify text-[#111111]/65">{r.text}</p>
        </div>
          ...
          <div className="mt-6 flex justify-center gap-2">
            {reviews.map((rev, i) => (
              <button
                ...
                style={{ backgroundColor: i === active ? GOLD : "rgba(17,17,17,0.2)" }}
              />
            ))}
          </div>
        )}
